    ...
    <div class="video">
        <div class="thumbnail">
            <a :href="'//www.nicovideo.jp/watch/' + video_id">
                ...
                <div
                    class="length_in_seconds"
                    v-if="length_in_seconds"
                >{{length_in_seconds|formatTime}}</div>
            </a>
        </div>

        <div class="main">
            <div class="title">
                <a :href="'//www.nicovideo.jp/watch/' + video_id">{{title}}</a>
            </div>
            <div class="count" :title="formatted_counts.detail">{{formatted_counts.rounded}}</div>
        </div>
    </div>
</template>
            ...
            const count = [
                {
                    ...
                    raw: this.view_count,
                    detail:
                        FormatHelper.separateByComma(this.view_count) + "視聴",
                ...
                },
                {
                    ...
                    detail:
                        FormatHelper.separateByComma(this.comment_count) +
                        "コメント",
                    rounded:
                        FormatHelper.roundedNumer(this.comment_count) + "コメ"
                },
                {
                    name: "mylist_count",
                    raw: this.mylist_count,
                    detail:
                        FormatHelper.separateByComma(this.mylist_count) +
                        "マイリス",
                    rounded:
                        FormatHelper.roundedNumer(this.mylist_count) + "マイ"
                },
                {
                    name: "posted_date",
                    raw: this.posted_date,
                    detail:
                        new Moment(this.posted_date).format(
                            "YYYY/MM/DD HH:mm"
                        ) + "投稿",
                ...
                },
                {
                    ...
                    detail:
                        "過去" +
                        FormatHelper.separateByComma(this.watch_count) +
                        "回視聴",
                    rounded:
                        "過去" +
                        FormatHelper.roundedNumer(this.watch_count) +
                        "回視聴"
                }
