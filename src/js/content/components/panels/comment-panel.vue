<template>
    <div class="panel">
        <template v-if="comments <= -3">読み込み中です。</template>
        <template v-else-if="comments < 0">読み込みに失敗しました。</template>
        <template v-else>
            <ul class="sorter">
                <li>
                    <NiicoCheckbox
            ...
            <div class="thread" ref="container" id="niico-comment-panel-scroll-event-target">
                <template v-for="(comment) in comments">
                    <section
                        class="comment"
                        :id="'niico-comment-panel-no-' + comment.no"
                        :key="comment.no"
                    >
                        <aside>
                            <div class="vpos property">
                                <a
                                    :title="comment.vpos|formatTime"
                                    :data-seektime="comment.vpos|formatTime"
                                >{{comment.vpos|formatTime}}</a>
                            </div>
                            <div class="mail property" :title="comment.mail">{{comment.mail}}</div>
                            <div class="no property" :title="'No.' + comment.no">No.{{comment.no}}</div>
                            <div
                                class="date property"
                                :title="comment.date|formartDate"
                            >{{comment.date|formartDate}}投稿</div>
                        </aside>
...
export default {
    ...
    methods: {
        ...
        comment_auto_scroll: function(current_time) {
            const container_el = document.getElementById(
                this.scroll_container_id
            );
            if (!container_el) return;
            const base = container_el.scrollTop - container_el.offsetTop;

            const el_to_scroll_id =
                this.comment_id_prefix + this.comment_no_to_scroll;
    ...
    mounted() {
        ...
        $(document)
            ...
            .on("wheel", e => {
                ...
                const wheel_target_is_comment_panel = $(e.target).parents(
                    "#" + this.scroll_container_id
                ).length;
