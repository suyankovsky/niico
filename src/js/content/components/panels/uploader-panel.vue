<template>
    <div class="panel">
        <template v-if="cannot_render_uploader_reason.code <= 0">
            {{cannot_render_uploader_reason.message}}
        </template>
        <template v-else>
            <section class="uploaderCardSection">
                ...
                <div class="details">
                    <div class="name">
                        <a class="ownerName" :href="uploader.href" target="_blank">
                            {{uploader.name}}
                        </a>
                    </div>
                    <div class="status">
                        ...
                        <template v-else>
                            <dl>
                                <dt>投稿動画：</dt>
                                <dd>
                                    <template v-if="!uploader.is_user_my_video_public">
                                        非公開
                                    </template>
                                    <template v-else>
                                        {{uploader.posted_video_all_count|loadCheck}}件
                                    </template>
                                </dd>
                            </dl>
                            <dl>
                                <dt>マイリス：</dt>
                                <dd>
                                    <template v-if="!uploader.is_user_open_list_public">
                                        非公開
                                    </template>
                                    <template v-else>
                                        {{public_mylist_keys.length|loadCheck}}件
                                    </template>
                                </dd>
                            </dl>
                        </template>
            ...
            <section class="selecterSection">
                <select v-model="active_list_id">
                    <option value="">
                        投稿動画（{{uploader.posted_video_all_count|loadCheck}}）
                    </option>
                    <optgroup
                        label="公開マイリスト"
                        v-if="public_mylist_keys.length > 0"
                    >
                        <template v-for="(item) in public_mylist_keys">
                            <option :value="item.id">{{item.name}}（{{item.count|loadCheck}}）</option>
                        </template>
                    </optgroup>
                </select>
            </section>
            ...
            <section class="selectedVideoListSection">
                <template v-if="selected_video_list_detail.code > 0">
                    <div class="meta">
                        {{selected_video_list_detail.count}}件
                    </div>
                    ...
                    <div class="more" v-if="active_list_id == '' && !uploader.posted_video_is_loaded_all">
                        <div class="button button--center" @click="pushPostedVideo(uploader)">
                            もっと見る
                        </div>
                    </div>
                </template>
                <template v-else>
                    {{selected_video_list_detail.content}}
                </template>
            </section>
        </template>
    </div>
    ...
    export default {
        ...
        computed: {
            ...
            ...mapGetters({
                ...
                cannot_render_uploader_reason: 'uploaders/cannot_render_uploader_reason',
            }),
            active_list_id: {
                get() {
