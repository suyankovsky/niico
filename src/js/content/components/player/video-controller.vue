<template>
    <div
        ...
        :id="'videoController--' + this.video_id"
    >
        <section class="contextActions" v-if="!is_mini_player_mode">
            <ul>
                <li class="button" @click="openTweet"><TwitterIcon /></li>
                <li class="button" @click="openNicoVideo"><ExternalLinkIcon /></li>
            </ul>
            <template v-if="viewerMylist.length > 1">
                <div class="mylist">
                    ...
                    />
                    <div class="button" @click="addMylist">
                        にマイリスト登録
                    </div>
                </div>
            </template>
        ...
        <section class="playerActions" v-if="!is_mini_player_mode">
            <div class="buttons">
                ...
                <div class="fastForward actionButton" @click="seekForward">
                    <div class="icon"><FastForwardIcon /></div>
                    <div class="text">{{setting.moves_seconds}}</div>
                </div>
            </div>
        </section>
                ...
                <li>
                    ...
                    <fieldset>
                        <template v-for="(value, key) in do_on_ended_map">
                            <button @click="changeDoOnEnded(key)" :class="[{'is-active': do_on_ended==key}]">
                                {{value}}
                            </button>
                        </template>
                    </fieldset>
...
<style lang="scss" scoped>
    ...
    .videoController {
        ...
        background: linear-gradient(to bottom, rgba(0, 0, 0, 0) 60%, rgba(0, 0, 0, 0.2) 90%, rgba(0, 0, 0, 0.8) 100%);
